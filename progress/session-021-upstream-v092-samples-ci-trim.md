...
- `tests/protocol/upstream_samples_tests.gd` (data-driven): every server
  ...
  form); published shapes that constrain the codec are pinned by name - `relay_type: "matchbox"` verbatim pass-through, `AuthorityResponse`
...
- `.llm/research/protocol-fixtures.md`: v0.9.2 refresh recorded (single
  ...
  PLAN section 3 warning about elided samples replaced with the new reality.
...
- Adversarial loop: round 1 (zero-knowledge red team) returned zero
  ...
  one declined per "simplify aggressively" (dpkg-query Status hardening - the failure mode is loud one step later). Round-2 re-review of the
