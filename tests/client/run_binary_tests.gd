...
# P2 binary game-data suite (PLAN §4.6): send_game_data_binary guards and wire
# bytes, strict envelope receive paths (bytes by default, opt-in MessagePack
# decode with raw-bytes fallback), rkyv pass-through, hostile-envelope
# hardening, and server format-downgrade reconciliation.
...
func _test_send_guards_and_wire_bytes() -> void:
	...
	# message_pack negotiation: bytes go out raw, backpressure applies.
	...
	_assert_equal(1, client.transport.sent_binary.size(), "backpressure drops binary")
	_assert_string_contains(errors[0], "backpressure", "binary backpressure message")
...
func _test_server_format_downgrade() -> void:
	# A server that downgrades the requested format (absent from
	# ProtocolInfo.game_data_formats, or an explicit UnsupportedGameDataFormat
	# error) pins the effective negotiation to JSON: binary sends refuse and
	# binary frames drop, matching what the server would do anyway.
	...
	downgrade_client.transport.inject_server_message(
		{"type": "ProtocolInfo", "data": unsupported_info}
	)
	_assert_equal(
		ERR_UNAVAILABLE,
	...
	downgrade_client.transport.inject_binary(
